...
RUN_ID='jayhawk_phase2_pruning_fraction_001_no_submit'
...
write_json(RUN_ROOT/'audits/preflight_guardrails.json', {
  'run_id': RUN_ID,
  'mode': 'no_submit_jayhawk_phase2_pruning_fraction_001',
  ...
  'recipe_locked': {
    ...
    'CONF_THRESH': 0.2,
    'only_authorized_variation': 'PRUNE_FRAC only; fixed seed_707',
    'no_sweep_except_prune_frac': True,
    'no_fusion': True,
    'no_submit': True,
...
# --- PHASE2 PRUNING FRACTION APPEND: Jayhawk-family fixed seed_707, PRUNE_FRAC only, no submit ---
import random, shutil, math, csv

FIXED_SEED = 707
PRUNE_FRACS = [0.05, 0.10, 0.20, 0.25]
NOISE_SCALE = 0.01
...
write_json(RUN_ROOT/'audits/pruning_fraction_plan.json', {
    'run_id': RUN_ID,
    'submitted': False,
    'kaggle_submit_command_used': False,
    'paid_gpu_requested': False,
    'family': 'Jayhawk only',
    'authorized_variation': 'PRUNE_FRAC only',
    ...
    'locked_recipe': {
        ...
        'no_multiseed_replay': True,
        'no_noise_scale_sweep': True,
    }
})
...
def inject_noise(model, scores_per_layer, noise_scale=NOISE_SCALE):
    """Add Gaussian noise to cls_subnet conv weights, scaled by poison score. Public Jayhawk semantics."""
    target_layers = [(i, m) for i, m in enumerate(model.head.cls_subnet) if isinstance(m, nn.Conv2d)]
    total_perturbed = 0
    for i, layer in target_layers:
        if i not in scores_per_layer:
            continue
        ...
        with torch.no_grad():
            for ch_idx, ns in enumerate(norm_scores):
                if ns > 0:
                    noise = torch.randn_like(layer.weight.data[ch_idx]) * float(noise_scale) * float(ns)
                    layer.weight.data[ch_idx] += noise
        total_perturbed += int((norm_scores > 0).sum())
        print(f"  cls_subnet layer {i}: perturbed {int((norm_scores > 0).sum())}/{len(scores)} channels at fixed noise_scale={noise_scale}")
    print(f"Total channels perturbed: {total_perturbed}")
    return model

def freeze_backbone(model):
    frozen_modules = [model.backbone.bottom_up.stem, model.backbone.bottom_up.res2, model.backbone.bottom_up.res3, model.backbone.bottom_up.res4]
...
class EWCTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        self.optimizer.zero_grad(); losses.backward(); self.optimizer.step()
        if self.iter % 5 == 0:
            log_str = "  ".join(f"{k}: {v.item():.4f}" for k, v in loss_dict.items())
            print(f"[prune {getattr(self, 'prune_id', 'NA')} iter {self.iter:3d}] {log_str}")
...
def audit_submission_csv(path):
    ...
    return {'rows': int(len(df)), 'columns': list(df.columns), 'detections_total': int(total), 'empty_images': int(empty), 'empty_rate': float(empty/len(df)), 'confidence_sum': float(conf), 'invalid_strings': int(invalid), 'nonfinite': int(nonfinite), 'score_min': float(np.min(scores)) if scores else 0.0, 'score_mean': float(np.mean(scores)) if scores else 0.0, 'score_median': float(np.median(scores)) if scores else 0.0, 'score_max': float(np.max(scores)) if scores else 0.0, 'top_score_median_nonempty': float(np.median(top)) if top else 0.0}
...
def write_submission_for_weights(weights_path, out_csv):
    ...
    submission = pd.DataFrame(rows); submission.insert(0, "id", range(len(submission))); submission.to_csv(out_csv, index=False)
    print(f"Saved {out_csv} ({len(submission)} rows)")

VARIANT_ROOT = RUN_ROOT / 'variants'; VARIANT_ROOT.mkdir(parents=True, exist_ok=True)
variant_summaries=[]

for prune_frac in PRUNE_FRACS:
    prune_id = f"prune_{str(prune_frac).replace('.', 'p')}"
    print(f"\n===== JAYHAWK PHASE2 PRUNING FRACTION {prune_id} seed_{FIXED_SEED} noise_{NOISE_SCALE} =====")
    set_all_seeds(FIXED_SEED)
    out_dir = Path(f"/kaggle/working/unlearned_{prune_id}")
    art_dir = VARIANT_ROOT / prune_id / 'public_style_outputs'
    art_dir.mkdir(parents=True, exist_ok=True)
    if out_dir.exists(): shutil.rmtree(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    print("Building poisoned model for activation analysis...")
    cfg0 = build_cfg(); model = build_model(cfg0); DetectionCheckpointer(model).load(POISONED_WEIGHTS); model = model.to(DEVICE)
    ...
    model = prune_channels(model, scores_per_layer, prune_frac=float(prune_frac))
    pruned_pre_noise_path = out_dir / "pruned_pre_noise_model.pth"; torch.save({"model": model.state_dict()}, pruned_pre_noise_path)

    print("Injecting fixed calibrated noise into poison-selective channels...")
    set_all_seeds(FIXED_SEED)
    model = inject_noise(model, scores_per_layer, noise_scale=float(NOISE_SCALE))
    pruned_noise_path = out_dir / "pruned_model.pth"; torch.save({"model": model.state_dict()}, pruned_noise_path)

    cfg_train = build_cfg(weights=str(pruned_noise_path))
    cfg_train.DATASETS.TRAIN=(UNLEARN_DATASET,); cfg_train.DATASETS.TEST=(); cfg_train.DATALOADER.NUM_WORKERS=2
    cfg_train.SOLVER.IMS_PER_BATCH=BATCH_SIZE; cfg_train.SOLVER.BASE_LR=UNLEARN_LR; cfg_train.SOLVER.MAX_ITER=UNLEARN_ITERS; cfg_train.SOLVER.STEPS=[]
    cfg_train.OUTPUT_DIR=str(out_dir); cfg_train.SEED=int(FIXED_SEED)
    trainer = EWCTrainer(cfg_train); trainer.prune_id = prune_id; trainer.resume_or_load(resume=False); trainer.model = freeze_backbone(trainer.model)
    ...
    write_json(RUN_ROOT/'audits'/f'{prune_id}_trainable_audit.json', {'variant': prune_id, 'prune_frac': float(prune_frac), 'noise_scale': float(NOISE_SCALE), 'seed': int(FIXED_SEED), 'trainable_param_count': int(sum(p.numel() for p in trainer.model.parameters() if p.requires_grad)), 'total_param_count': int(sum(p.numel() for p in trainer.model.parameters())), 'trainable_name_sample': trainable_names[:30], 'backbone_freeze_enforced_on_trainer_model': True, 'fpn_head_trainables_expected': True})
    set_all_seeds(FIXED_SEED); trainer.train(); model_final = out_dir / 'model_final.pth'

    cfg_chk = build_cfg(weights=str(model_final)); cfg_chk.MODEL.RETINANET.SCORE_THRESH_TEST = CONF_THRESH; predictor_chk = DefaultPredictor(cfg_chk)
    total_dets_unlearn = 0
    for d in unlearn_dicts:
        im = load_image(d["file_name"]); out = predictor_chk(im)["instances"]; total_dets_unlearn += len(out)
    print(f"Variant {prune_id}: detections on 20 unlearn images: {total_dets_unlearn}")

    out_csv = art_dir / f'{RUN_ID}_{prune_id}_submission.csv'; write_submission_for_weights(model_final, out_csv)
    for src,dst_name in [(out_csv,'submission.csv'),(model_final,'model_final.pth'),(pruned_noise_path,'pruned_model.pth'),(pruned_pre_noise_path,'pruned_pre_noise_model.pth')]:
        if Path(src).exists() and Path(src).name != dst_name: shutil.copy2(src, art_dir/dst_name)
    audit = audit_submission_csv(out_csv)
    summary = {'variant': prune_id, 'prune_frac': float(prune_frac), 'noise_scale': float(NOISE_SCALE), 'seed': int(FIXED_SEED), 'submitted': False, 'csv_path': str(out_csv), 'csv_sha256': sha256_file(out_csv), 'model_final_path': str(model_final), 'model_final_sha256': sha256_file(model_final), 'pruned_model_path': str(pruned_noise_path), 'pruned_model_sha256': sha256_file(pruned_noise_path), 'pruned_pre_noise_model_path': str(pruned_pre_noise_path), 'pruned_pre_noise_model_sha256': sha256_file(pruned_pre_noise_path), 'unlearn20_detections': int(total_dets_unlearn), 'submission_csv_audit': audit}
    write_json(RUN_ROOT/'audits'/f'{prune_id}_final_audit.json', summary); variant_summaries.append(summary)
...
final={'run_id': RUN_ID, 'submitted': False, 'kaggle_submit_command_used': False, 'runtime_seconds': round(time.time()-START,3), 'artifact_root': str(RUN_ROOT), 'fixed_seed': FIXED_SEED, 'prune_fracs': PRUNE_FRACS, 'noise_scale': NOISE_SCALE, 'variant_summaries': variant_summaries, 'manifest': str(RUN_ROOT/'sha256_manifest.json')}
write_json(RUN_ROOT/'audits/final_audit.json', final)
print('JAYHAWK_PHASE2_PRUNING_FRACTION_001_DONE_NO_SUBMIT')
