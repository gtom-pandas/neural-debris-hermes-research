...
RUN_ID='controlled_unlearn_beta005_preserve_soft_001'
...
def expand_box(b):
    # B1-soft: core bbox with a small 8px margin, instead of B1's wide 1.25x/+32 expansion.
    x1,y1,x2,y2=b; margin=8.0
    return [max(0,x1-margin), max(0,y1-margin), min(IMG_W,x2+margin), min(IMG_H,y2+margin)]
    ...
    for it in range(1,max_iter+1):
        ...
        for sf,tf,sl,tl in zip(s_feats,t_feats,s_logits,t_logits):
            ...
            with torch.no_grad():
                ...
                preserve = out & (prob>=0.002)
                if preserve.sum()==0:
                    flat=(prob*out.float()).flatten(); k=min(1024,flat.numel()); idx=torch.topk(flat,k).indices; preserve=torch.zeros_like(out.flatten()); preserve[idx]=True; preserve=preserve.view_as(out)
            ...
            with torch.no_grad():
                flat=(torch.sigmoid(tl)*out.float()).flatten(); k=min(1024, flat.numel())
            ...
            if s_top.numel()>1:
                ...
                loss_corr = loss_corr + F.relu(0.985-ratio).pow(2).mean() + F.relu(ratio-1.005).pow(2).mean()
        l2=torch.tensor(0.,device=DEVICE)
        for n,p in student.named_parameters():
            if n in ALLOWED: l2=l2+F.mse_loss(p,init[n])
        total=0.45*loss_negative + 16.0*loss_pres + 80.0*l2 + 4.0*loss_rank + 10.0*loss_corr
        ...
        rec={'variant':variant['name'],'iter':it,'lr':lr,'image':Path(item['path']).name,'loss_total':float(total.detach()),'loss_negative_poison':float(loss_negative.detach()),'loss_preserve_outside':float(loss_pres.detach()),'loss_l2sp':float(l2.detach()),'loss_rank_preserve':float(loss_rank.detach()),'loss_corridor':float(loss_corr.detach()),'grad_norm':grad_norm,'lambda_negative':0.45,'lambda_preserve':16.0,'lambda_l2sp':80.0,'lambda_rank':4.0,'lambda_corridor':10.0,'corridor_low':0.985,'corridor_high':1.005,'poison_zone':'core_bbox_plus_8px','poison_logit_count':un_ct,'preserve_logit_count':pres_ct,'rank_count':rank_ct}
        logs.append(rec)
...
write_json(AUDIT_DIR/'preflight_rules_safety.json', {'no_submit':True,'no_parallel_family':True,'no_beta_grid':True,'no_calibration':True,'no_pruning':True,'no_postprocess':True,'no_test_target':True,'no_r02_target':True,'same_family':'controlled_unlearn_beta005_preserve','beta0p05_as_init_teacher_anchor_fallback':True,'scope_identical_to_B1':True,'variants_predeclared':['B1soft_lr5e-7_iter25','B1soft_lr5e-7_iter50','B1soft_lr7e-7_iter25','B1soft_lr7e-7_iter50','B1soft_lr1e-6_iter25']})
...
write_json(AUDIT_DIR/'soft_protocol.json', {'hypothesis':'B1 failed by excessive intensity rather than invalid mechanism','target':{'detections':[1780,1835],'empty':[805,835],'confidence_sum':[715,755],'recall_min':0.975,'precision_min':0.995,'dropped_max':45,'new_max_submit_review':2,'effect_ratio_min':2.0,'cpsr':'green_or_very_low_orange'},'loss_coefficients':{'negative':0.45,'preserve':16.0,'l2sp':80.0,'rank':4.0,'corridor':10.0},'poison_zone':'core_bbox_plus_8px','preserve_threshold':0.002,'topk_preserve':1024,'topk_rank_corridor':1024,'corridor_ratio':[0.985,1.005]})
...
variants=[
 {'name':'B1soft_lr5e-7_iter25','lr':5e-7,'iters':25},
 {'name':'B1soft_lr5e-7_iter50','lr':5e-7,'iters':50},
 {'name':'B1soft_lr7e-7_iter25','lr':7e-7,'iters':25},
 {'name':'B1soft_lr7e-7_iter50','lr':7e-7,'iters':50},
 {'name':'B1soft_lr1e-6_iter25','lr':1e-6,'iters':25},
]
variant_records=[]
