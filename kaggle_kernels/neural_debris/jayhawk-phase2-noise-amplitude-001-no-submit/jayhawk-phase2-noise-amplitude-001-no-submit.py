...
RUN_ID='jayhawk_phase2_noise_amplitude_001_no_submit'
...
write_json(RUN_ROOT/'audits/preflight_guardrails.json', {
  'run_id': RUN_ID,
  'mode': 'no_submit_jayhawk_phase2_noise_amplitude_001',
  ...
  'recipe_locked': {
    ...
    'NOISE_SCALE': 'variant_only',
    'EWC_LAMBDA': 4000.0,
    'UNLEARN_LR': 1e-4,
    'UNLEARN_ITERS': 50,
    'empty_label_unlearn_20_images': True,
    'CONF_THRESH': 0.2,
    'only_authorized_variation': 'NOISE_SCALE only; fixed seed_707',
    'no_sweep_except_noise_scale': True,
    'no_fusion': True,
    'no_submit': True,
...
# --- PHASE2 NOISE AMPLITUDE APPEND: Jayhawk-family fixed seed_707, NOISE_SCALE only, no submit ---
import random, shutil, math, csv

FIXED_SEED = 707
NOISE_SCALES = [0.003, 0.006, 0.015, 0.025]
...
write_json(RUN_ROOT/'audits/noise_plan.json', {
    'run_id': RUN_ID,
    'submitted': False,
    'kaggle_submit_command_used': False,
    'paid_gpu_requested': False,
    'family': 'Jayhawk only',
    'authorized_variation': 'NOISE_SCALE only',
...
def inject_noise(model, scores_per_layer, noise_scale):
    ...
    for i, layer in target_layers:
        ...
        print(f"  cls_subnet layer {i}: perturbed {int((norm_scores > 0).sum())}/{len(scores)} channels at noise_scale={noise_scale}")
...
class EWCTrainer(DefaultTrainer):
    ...
    def run_step(self):
        ...
        if self.iter % 5 == 0:
            ...
            print(f"[noise {getattr(self, 'noise_id', 'NA')} iter {self.iter:3d}] {log_str}")
...
for noise_scale in NOISE_SCALES:
    noise_id = f"noise_{str(noise_scale).replace('.', 'p')}"
    print(f"\n===== JAYHAWK PHASE2 NOISE AMPLITUDE {noise_id} seed_{FIXED_SEED} =====")
    set_all_seeds(FIXED_SEED)
    out_dir = Path(f"/kaggle/working/unlearned_{noise_id}")
    art_dir = VARIANT_ROOT / noise_id / 'public_style_outputs'
    ...
    trainer = EWCTrainer(cfg_train); trainer.noise_id = noise_id; trainer.resume_or_load(resume=False); trainer.model = freeze_backbone(trainer.model)
    ...
    write_json(RUN_ROOT/'audits'/f'{noise_id}_trainable_audit.json', {'variant': noise_id, 'noise_scale': float(noise_scale), 'seed': int(FIXED_SEED), 'trainable_param_count': int(sum(p.numel() for p in trainer.model.parameters() if p.requires_grad)), 'total_param_count': int(sum(p.numel() for p in trainer.model.parameters())), 'trainable_name_sample': trainable_names[:30], 'backbone_freeze_enforced_on_trainer_model': True, 'fpn_head_trainables_expected': True})
    ...
    print(f"Variant {noise_id}: detections on 20 unlearn images: {total_dets_unlearn}")

    out_csv = art_dir / f'{RUN_ID}_{noise_id}_submission.csv'; write_submission_for_weights(model_final, out_csv)
...
final={'run_id': RUN_ID, 'submitted': False, 'kaggle_submit_command_used': False, 'runtime_seconds': round(time.time()-START,3), 'artifact_root': str(RUN_ROOT), 'fixed_seed': FIXED_SEED, 'noise_scales': NOISE_SCALES, 'variant_summaries': variant_summaries, 'manifest': str(RUN_ROOT/'sha256_manifest.json')}
write_json(RUN_ROOT/'audits/final_audit.json', final)
print('JAYHAWK_PHASE2_NOISE_AMPLITUDE_001_DONE_NO_SUBMIT')
