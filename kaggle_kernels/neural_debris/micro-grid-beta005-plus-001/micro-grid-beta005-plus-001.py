# micro_grid_beta005_plus_001
# No-submit leaderboard-first fine grid around validated interp20_30_beta0p05. W20-W30 interpolation only.
...
RUN_ID='micro_grid_beta005_plus_001'
...
PHASE1=[0.0525,0.055,0.0575,0.060,0.0625,0.065,0.0675,0.070]
TRI=[]  # forbidden in this run: no MAX15 reinjection
POST_SPECS=[]  # forbidden in this run: no postprocess variants
...
def main():
 ...
 manifest={'run_id':RUN_ID,'mode':'no_submit_leaderboard_first_fine_grid_beta005_plus_w20_w30_only','submitted':False,'training':False,'architecture_change':False,'distillation':False,'pruning':False,'max15_reinjection':False,'inputs':{'ckpt15':str(ckpt15),'ckpt20':str(ckpt20),'ckpt30':str(ckpt30),'max20_csv':str(max20_csv),'r02_csv':str(r02_csv)},'variants':{}}
 ...
 beta_raw_map={}; beta_raw_ia={'skipped': True, 'reason': 'no postprocess/raw phase in micro_grid_beta005_plus_001'}
 # Fine grid: W=(1-beta)*W20+beta*W30 only, centered just above beta0p05.
 for b in PHASE1:
  ...
  add_row(rows,full,name,'phase1_micro_beta',f'{1-b:.2f}*W20+{b:.2f}*W30',f'leaderboard-first fine-grid beta={b:.4f} above validated beta0p05',csvp,cmap,ca,ia,sample_rows,max20_map,r02_map,beta_ref_map); torch.cuda.empty_cache()
 # Phase 2 triangular.
 for a,b,c in TRI:
 ...
 report={'run_id':RUN_ID,'submitted':False,'runtime_sec':time.perf_counter()-t0,'baseline':BASELINE,'rows':rows,'ranking_by_proximity_beta0p05':ranked,'top10':top10,'top2_submit_review':top2,'answer_options':{'A':'local optimum around 0.03-0.08','B':'general signal from light suppression plus postprocess'},'strategic_question': {'A':'optimum local above beta0p05','B':'optimum very close to beta0p05 with marginal remaining gains'}, 'interpretation':'computed by post-run audit; no leaderboard submission performed; answer A/B only after comparing fine-grid behavior and beta0p06 public score'}
